...
function normalizeBag(raw) {
  const out = { stacks: {}, equipped: { weaponMage: null, weaponWarrior: null, ring1: null, ring2: null, amulet: null, armor: null, helmet: null }, enchants: {} };
  ...
  for (const slot of ["weaponMage", "weaponWarrior", "ring1", "ring2", "amulet", "armor", "helmet"]) {
    const key = eq[slot];
    ...
    } else if (item.slot !== "amulet" && item.slot !== "weapon" && item.slot !== "armor" && item.slot !== "helmet") out.equipped[slot] = key; // ring slots
  }
...
async function runEconomyAction(action, profileId, payload) {
  ...
  if (action === "equip") {
    ...
    // Class-flavoured armor/headgear (mage robes/hats) can't be worn by the wrong class.
    if ((item.slot === "armor" || item.slot === "helmet") && item.charClass) {
  ...
  if (action === "unequip") {
    ...
    if (!["weaponMage", "weaponWarrior", "ring1", "ring2", "amulet", "armor", "helmet"].includes(slot)) return fail("Bad slot");
...
function gearXpBonus(profile) {
  ...
  const keys = [bag.equipped[wslot], bag.equipped.ring1, bag.equipped.ring2, bag.equipped.amulet, bag.equipped.armor, bag.equipped.helmet];
  let bonus = 0;
